...
// A*-Search for solving the 8 Puzzle using the weighted Manhattan Distances considering
// the value of each piece as its cost per move
public class AStar3 {
	...
	int maxQueueSize = 0;	// variable contains the largest priorityQ size during search
		...
		public AStar3(BoardState inBoard){
		...
		// A*3-Search function
		public void search(){
			long aStar3StartTime = System.currentTimeMillis();		// time marker for beginning of search
				...
				if (!temp.getCurrentState().isGoalState()){	// checks if goal state has been found, executes if not
					...
					for (int i = 0; i < successors.size(); i++){	// loop through all successors
						...
						if (!explored.contains(intVal)){	// checks if state has already been explored
							...
							int currentHCost = temp.getCurrentState().getWeightedManhattanDistances();	// current weighted Manhattan Distance heuristic stored
					...
					long aStar3EndTime = System.currentTimeMillis();	// time marker for when goal state has been found
					double aStar3Time = aStar3EndTime - aStar3StartTime;	// total time from start of search to when goal state was found
					...
					System.out.println("**********************************************************");
					System.out.println("*                    A*3 SOLUTION PATH                   *");
					System.out.println("**********************************************************");
					System.out.println();
					...
					//System.out.println("A*3 took " + (aStar3Time / 1000) + " seconds to find goal state");
					System.out.println("**********************************************************");
					System.out.println();
					System.out.println();
					System.out.println();
