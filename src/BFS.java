...
public class BFS {
	...
	Queue<Node> queue = new LinkedList<Node>();	// FIFO queue for successors of states
	int maxQueueSize = 0;	// variable contains the largest queue size during search
		
		// Constructor takes in a BoardState
		public BFS(BoardState inBoard){

			Node rootNode = new Node(inBoard);	// root node created with passed initial state
			queue.add(rootNode);	// root node added to queue
		...
		public void search(){
			long BFSStartTime = System.currentTimeMillis();		// time marker for beginning of search
			int nodesPopped = 0;	// variable tracks number of nodes popped off queue
			while (!queue.isEmpty()){	// while loop executes while the queue is not empty
				Node temp = queue.poll();	// leading node of queue is removed and stored in temp
				...
				if (!temp.getCurrentState().isGoalState()){	// checks if goal state has been found, executes if not
					...
					for (int i = 0; i < successors.size(); i++){	// loop through all successors
						...
						if (!explored.contains(intVal)){	// checks if state has already been explored
							...
							queue.add(child);	// child node added to queue
							if (maxQueueSize < queue.size()){	// checks if queue is larger than previous max
								maxQueueSize = queue.size();	// assigns new max
							}
						}
					}
				}else{	// executes when goal state has been found
					long BFSEndTime = System.currentTimeMillis();		// time marker for when goal state has been found
					double BFSTime = BFSEndTime - BFSStartTime;		// total time from start of search to when goal state was found
					...
					//System.out.println("BFS took " + (BFSTime / 1000) + " seconds to find the goal state");
					System.out.println("**************************************");
					System.out.println();
					System.out.println();
					System.out.println();
