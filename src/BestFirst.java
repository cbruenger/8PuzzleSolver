...
// Best-First-Search for solving the 8 Puzzle using the number of pieces out
// of place as the heuristic
public class BestFirst {
	...
	int maxQueueSize = 0;	// variable contains the largest priorityQ size during search
		...
		public BestFirst(BoardState inBoard){

			Node rootNode = new Node(inBoard);	// root node created with passed initial state
			rootNode.isBestFirstNode = true;	// node type set for comparable
		...
		public void search(){
			long bestFirstStartTime = System.currentTimeMillis();		// time marker for beginning of search
				...
				if (!temp.getCurrentState().isGoalState()){	// checks if goal state has been found, executes if not
					...
					for (int i = 0; i < successors.size(); i++){	// loop through all successors
						...
						if (!explored.contains(intVal)){	// checks if state has already been explored
							...
							int currentHCost = temp.getCurrentState().getPiecesOutOfPlace();	// current pieces out of place heuristic stored
							...
							child.isBestFirstNode = true;	// node type set for comparable
					...
					long bestFirstEndTime = System.currentTimeMillis();		// time marker for when goal state has been found
					double bestFirstTime = bestFirstEndTime - bestFirstStartTime;	// total time from start of search to when goal state was found
					...
					System.out.println("***********************************************************");
					System.out.println("*                 BEST FIRST SOLUTION PATH                *");
					System.out.println("***********************************************************");
					System.out.println();
					...
					System.out.println("**********************************************************");
					System.out.println("Search = BEST FIRST");
					...
					//System.out.println("Best-First took " + (bestFirstTime / 1000) + " seconds to find goal state");
					System.out.println("**********************************************************");
					System.out.println();
					System.out.println();
					System.out.println();
