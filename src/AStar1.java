...
// A*-Search for solving the 8 Puzzle using the number of pieces out of place
// as the heuristic
public class AStar1 {
	...
	int maxQueueSize = 0;	// variable contains the largest priorityQ size during search
		...
		public AStar1(BoardState inBoard){
		...
		// A*1-Search function
		public void search(){
			long aStar1StartTime = System.currentTimeMillis();		// time marker for beginning of search
			...
			while (!priorityQ.isEmpty()){	// while loop executes while the queue is not empty
				...
				if (!temp.getCurrentState().isGoalState()){	// checks if goal state has been found, executes if not
					...
					long aStar1EndTime = System.currentTimeMillis();		// time marker for when goal state has been found
					double aStar1Time = aStar1EndTime - aStar1StartTime;	// total time from start of search to when goal state was found
					...
					System.out.println("**********************************************************");
					System.out.println("*                    A*1 SOLUTION PATH                   *");
					System.out.println("**********************************************************");
					System.out.println();
					...
					//System.out.println("A*1 took " + (aStar1Time / 1000) + " seconds  to find goal state");
					System.out.println("**********************************************************");
					System.out.println();
					System.out.println();
					System.out.println();
